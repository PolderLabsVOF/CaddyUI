...
function omitEmpty(value) {
  ...
  return Object.fromEntries(Object.entries(value)
    .map(([key, child]) => [key, omitEmpty(child)])
    .filter(([, child]) => child !== '' && child !== undefined && child !== null));
}

export default function CaddyRuntime({ api, canEdit, theme, notify }) {
  ...
  const healthyUpstreams = upstreams.filter((item) => !item.unhealthy && !item.unavailable && Number(item.fails || 0) === 0).length;
  const features = [
  ...
  const saveJsonPath = async () => {
    ...
    try {
      if (selectedPath.length === 0) {
        await api('/api/caddy/load', { method: 'POST', body: JSON.stringify({ content: value, format: 'json' }) });
      } else {
      ...
      setError(saveError.message.includes('412') ? 'The live config changed. Refresh and reapply your edit.' : saveError.message);
    } finally { setBusy(false); }
  };

  const openServerEditor = (name, server) => {
    setServerEdit({
      name,
      original: server,
      listen: (server.listen || []).join(', '),
      protocols: server.protocols || ['h1', 'h2', 'h3'],
      ...
      keepalive_idle: server.keepalive_idle || '',
      keepalive_count: server.keepalive_count ?? '',
      max_header_bytes: server.max_header_bytes ?? '',
      strict_sni_host: server.strict_sni_host === undefined ? 'default' : String(server.strict_sni_host),
      allow_0rtt: server.allow_0rtt === undefined ? 'default' : String(server.allow_0rtt),
      enable_full_duplex: Boolean(server.enable_full_duplex),
    });
  };
  ...
  const saveServer = async (event) => {
    ...
    const next = {
      ...serverEdit.original,
      listen: serverEdit.listen.split(',').map((item) => item.trim()).filter(Boolean),
      protocols: serverEdit.protocols,
      read_header_timeout: serverEdit.read_header_timeout,
      read_timeout: serverEdit.read_timeout,
      write_timeout: serverEdit.write_timeout,
      idle_timeout: serverEdit.idle_timeout,
      keepalive_interval: serverEdit.keepalive_interval,
      keepalive_idle: serverEdit.keepalive_idle,
      keepalive_count: serverEdit.keepalive_count === '' ? undefined : Number(serverEdit.keepalive_count),
      max_header_bytes: serverEdit.max_header_bytes === '' ? undefined : Number(serverEdit.max_header_bytes),
      strict_sni_host: serverEdit.strict_sni_host === 'default' ? undefined : serverEdit.strict_sni_host === 'true',
      allow_0rtt: serverEdit.allow_0rtt === 'default' ? undefined : serverEdit.allow_0rtt === 'true',
      enable_full_duplex: serverEdit.enable_full_duplex || undefined,
    };
    ...
    try {
      await api(apiPath(['apps', 'http', 'servers', serverEdit.name]), {
        method: 'PATCH',
        headers: etag ? { 'If-Match': etag } : {},
        body: JSON.stringify({ value: omitEmpty(next) }),
      });
  ...
  return <section className="runtime-page">
    ...
    {config && tab === 'overview' && <>
      <div className="runtime-kpis">
        ...
        <Metric label="Upstreams healthy" value={`${healthyUpstreams}/${upstreams.length}`} detail="Caddy passive health state" />
      ...
      <div className="runtime-grid">
        ...
        <Panel title="Installed app modules" subtitle="Every app remains editable in the JSON API">
    ...
    {config && tab === 'observability' && <div className="runtime-grid observability-grid">
      ...
      <Panel title="Reverse proxy upstreams" subtitle="Live passive health and request counters">
        <div className="upstream-table">{upstreams.map((upstream, index) => <div key={`${upstream.address || upstream.dial || 'upstream'}-${index}`}><span className={`runtime-dot ${!upstream.unhealthy && !upstream.unavailable ? 'on' : ''}`} /><strong>{upstream.address || upstream.dial || 'Unnamed upstream'}</strong><span>{upstream.num_requests ?? upstream.requests ?? 0} active</span><span>{upstream.fails ?? 0} failures</span></div>)}</div>
    ...
    {config && tab === 'json' && <div className="json-workbench">
      ...
      <section className="json-editor-panel">
        <div className="json-editor-head"><div><p className="eyebrow">Selected API path</p><h2 className="mono">/config/{selectedPath.map((part) => `${part}/`).join('')}</h2></div><div><button onClick={() => selectTreePath(selectedPath)} disabled={!editorDirty}>Discard</button><button className="primary" onClick={saveJsonPath} disabled={!canEdit || busy || !editorDirty}>{busy ? <Loader2 size={15} className="spin" /> : <CheckCircle2 size={15} />}Save selected path</button></div></div>
        ...
        <p className="json-help">Writes are applied atomically to the live Admin API. ETag matching prevents overwriting a configuration changed by another operator or process.</p>
      </section>
    </div>}
